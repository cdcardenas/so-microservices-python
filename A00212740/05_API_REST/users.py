...
from users_commands import get_all_users, add_user, remove_user, get_that_user
...
def get_single_user(username):
 usuario = get_that_user(username)[0]
 if len(usuario)==0:
     return "el usuario no existe", 400
 that = {}
 that["nombre de usuario"] = usuario[1]
 that["directorio de trabajo"]= usuario[2]
 return json.dumps(that), 200
